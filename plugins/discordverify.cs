...
namespace Core
{
    public class DiscordVerify : Plugin
    {
        ...
        public string serverID = "943304186395766865"; // The ID of the server
        public bool giveRole = true; // Whether the player will be given a role (true) or not (false) upon verifying
        public string roleID = "944816931129667634"; // The ID of the role to give if giveRole is true
        public bool changeNick = true; // Whether the player's nick will be forcefully changed to their username (true) or not (false) upon verifying
    ...
    public sealed class CmdVerify : Command2
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            if (data != null)
            {
                p.Message("%~You are already verified on the server.");
                return;
            }

            string code = GetVerificationCode(8);

            p.Message("%~Type %e.verify " + code + " %~on the verify channel on the discord server to be verified.");
        ...
        public override void Help(Player p)
        {
            ...
            p.Message("%HType %b.verify [code] %Hon the discord server to verify your account.");
        }
    }
}